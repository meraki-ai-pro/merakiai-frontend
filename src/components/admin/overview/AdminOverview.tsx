...
function StatCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border bg-card p-5 hover:bg-accent transition-colors">
...
function SectionHeader({ title, href }: { title: string; href: string }) {
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      <Link
        href={href}
        className="flex items-center gap-1 text-xs text-primary/70 hover:text-primary/80 transition-colors"
      >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {recentUsers.length === 0 ? (
            <p className="text-xs text-muted-foreground/60 py-6 text-center">No users data</p>
          ) : (
            <ul className="space-y-2">
              {recentUsers.map((user) => (
                <li key={user.id} className="flex items-center gap-3 rounded-lg p-2 hover:bg-card transition-colors">
                  ...
                  {user.role === 'admin' && (
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-primary/20 text-primary">
                      ADMIN
                    </span>
                  )}
                </li>
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {recentSessions.length === 0 ? (
            <p className="text-xs text-muted-foreground/60 py-6 text-center">No sessions data</p>
          ) : (
            <ul className="space-y-2">
              {recentSessions.map((session) => (
                <li key={session.id} className="rounded-lg p-2 hover:bg-card transition-colors">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-1 text-[10px] text-muted-foreground/70">
                      ...
                      {session.ended_at ? (
                        <span className="text-muted-foreground/60">ended</span>
                      ) : (
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          {recentFeedback.length === 0 ? (
            <p className="text-xs text-muted-foreground/60 py-6 text-center">No feedback data</p>
          ) : (
            <ul className="space-y-2">
              {recentFeedback.map((fb) => (
                <li key={fb.id} className="rounded-lg p-2 hover:bg-card transition-colors">
                  <div className="flex items-center justify-between gap-2 mb-1">
      ...
      <div className="rounded-xl border border-primary/15 bg-primary/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-primary/80 mb-1">Admin API Note</p>
            <p className="text-xs text-primary/80/60 leading-relaxed">
              The admin dashboard reads from <code className="bg-primary/15 px-1 rounded">/admin/*</code> endpoints
              that require <code className="bg-primary/15 px-1 rounded">admin_guard</code> on the backend.
              Document upload uses the existing <code className="bg-primary/15 px-1 rounded">/ingestion/documents</code> endpoint (already admin-guarded).
              Ensure your Supabase <code className="bg-primary/15 px-1 rounded">users</code> table has a{' '}
              <code className="bg-primary/15 px-1 rounded">role</code> column with{' '}
              <code className="bg-primary/15 px-1 rounded">&apos;admin&apos;</code> value for admin accounts.
            </p>
          </div>
        </div>
      </div>
